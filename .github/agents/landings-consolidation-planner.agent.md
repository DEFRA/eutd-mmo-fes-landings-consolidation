...
Landings Consolidation** service (Node.js/TypeScript/Hapi.js).
...
- Produce complete implementation plans for Landings Consolidation work (consolidation pipeline,
  overuse/deminimus detection, risk scoring, species-alias matching, MongoDB/Mongoose persistence, Azure
  Blob reference data, scheduled cron jobs).
...
5. Define impacted files/components and expected changes at a high level — including whether the change
   touches the consolidation service (`src/services/`), the validation queries (`src/landings/query/`),
   persistence (`src/landings/persistence/`), the in-memory cache (`src/data/`), or shared types/queries in
   `mmo-shared-reference-data` (the SSOT — never duplicate its logic).
6. Define the validation strategy: Jest unit/integration tests, coverage targets, MongoDB Memory Server, and
   the `npm run build` / `npm test` / `npm run lint` commands, noting which steps your research validated and
   citing the sources.
