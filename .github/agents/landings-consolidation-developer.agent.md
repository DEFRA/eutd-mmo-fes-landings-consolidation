...
- **Invoked standalone without a plan?** Apply the framework's triage:
  ...
  - **Complex** (new architecture, overuse/deminimus or risk-scoring rule changes, a new external
    integration, scheduled-job changes, a security surface) — delegate planning to the
    [Planner - Landings Consolidation](landings-consolidation-planner.agent.md), do **not** author it
    yourself, then present it and obtain user approval before implementing.
