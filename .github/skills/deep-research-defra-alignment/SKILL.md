...
description: "Do thorough, risk-scoped research in the open and align findings to the DEFRA standards precedence (DEFRA > GDS > community) for the MMO FES Landings Consolidation service. Use for the single, risk-scoped Research (§4.2) stage of the working framework — validating APIs, libraries, patterns, security and policy against DEFRA/GDS and framework guidance, and citing sources before a plan is approved or implemented."
argument-hint: "e.g. 'validate the deminimus tolerance edge cases the planner flagged' or 'research MongoDB aggregation vs in-memory consolidation for overuse detection'"
license: OGL-UK-3.0
metadata:
  author: mmo-fes
  version: "1.0"
...
- **Planner - Landings Consolidation** runs this single research pass for **Complex** work and cites
  sources directly in its plan.
- **Developer - Landings Consolidation** runs this same single pass for **Standard** work (or when invoked
...
Match effort to consequence. Go deeper the closer a step is to: **security / secrets / PII**, **landing-data
correctness** (overuse detection, deminimus 50kg tolerance, risk scoring, weight comparisons), **species-alias
matching**, **persistence** (MongoDB/Mongoose, aggregations), **external integrations** (Azure Blob reference
data, Service Bus, vessel service, Application Insights), **scheduled jobs** (node-cron cache refresh), or a
**version-sensitive API/library**. A cosmetic or well-trodden step needs little or none.
...
- Search **authoritative, current** sources: DEFRA & GOV.UK standards and service manuals, OWASP, the
  framework/library's own docs (Hapi, Mongoose, `@azure/service-bus`, `@azure/storage-blob`, `node-cron`,
  `moment`), and `mmo-shared-reference-data`. Prefer primary sources over blog posts.
...
- [ ] **Structured logging** — bracketed context tags (`[LANDINGS-CONSOLIDATION][ACTION][DETAIL]`) with
      correlation-id propagation.
- [ ] **Data correctness** — consolidation, overuse/deminimus detection, risk scoring and species-alias
      matching preserve integrity; deminimus 50kg tolerance and cached weighting are applied correctly.
...
**blocked**); send **revise/blocked** items back to the **Planner - Landings Consolidation** rather than
